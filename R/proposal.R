...
Proposal$set("public",
             ...
             function(blocks){
               ...
               private$.block_index_map <- map(private$.block_levels,
                                         ~ {
                                           which(blocks == .x)
                                         }) %>%
...
Proposal$set("public", "calc_acpt_rt",
         function(cur_mcmc_iter, acpt_ind, block_level){
           private$.acpt_rt[[block_level]] <-
             (private$.acpt_rt[[block_level]]*(cur_mcmc_iter - 1) + acpt_ind)/cur_mcmc_iter
           invisible(self)
         }
)
...
Proposal$set("public", "set_cur_block_info",
             function(block_level) {
               ...
               self$block_info$index <- private$.block_index_map[[block_level]]
               self$block_info$prop_var <- private$.prop_var[[block_level]]
               invisible(self)
             }
)
